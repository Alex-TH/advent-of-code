...
int main() {
  ...
  for (int i = 0; i < distances_len; i++) {
    ...
    // for (int x = 0; x < MAX_BOXES; x++) printf("%d ", circuit_map[x]);
    // printf("\n");
    printf("circuits len -> %d\n", circuits_len);
    for (int x = 0; x < MAX_BOXES; x++) printf("%d ", circuits[x]);
    printf("\n");
    printf("Total = %d\n", total);
    if (circuits_len == 1 && circuits[0] >= boxes_len){
      printf("Founded at %d! %ld X %ld\n", i, current.from.x, current.to.x);
      solution = current.from.x * current.to.x;
      break;
    }
  }
