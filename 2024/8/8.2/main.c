  ...
  for (int i = 0; i < ANTHENA_HASH_LEN; i++) {
    ...
    for (int a = 0; a < anthena_lists[i]->len - 1; a++) {
      for (int b = a + 1; b < anthena_lists[i]->len; b++) {
        ...
        char anthena_value = anthenas[a].value;

        while (is_in_bounds(antinode_a)) {
          if (board[antinode_a.y][antinode_a.x] == '.') {
            solution++;
            board[antinode_a.y][antinode_a.x] = antinode_a.value;
          }

          antinode_a.x -= distances[distance_index].x;
          antinode_a.y -= distances[distance_index].y;
        }
        while (is_in_bounds(antinode_b)) {
          if (board[antinode_b.y][antinode_b.x] == '.') {
            solution++;
            board[antinode_b.y][antinode_b.x] = antinode_b.value;
          }

          antinode_b.x += distances[distance_index].x;
          antinode_b.y += distances[distance_index].y;
          if (anthena_value == 'A') printf("{%d, %d}\n", antinode_b.x, antinode_b.y);
        }
