...
int calculate_score(unsigned short board[BOARD_HEIGHT][BOARD_WIDTH], Position start) {
  ...
  unsigned short visited[BOARD_HEIGHT * BOARD_WIDTH] = {0};
  ...
  while (len > 0) {
    ...
    if (current.height == 9 && !visited[(current.y * BOARD_HEIGHT) + current.x]){
      score++;
      visited[(current.y * BOARD_HEIGHT) + current.x] = 1;
      continue;
    }
